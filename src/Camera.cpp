...
namespace Azul
{
	...
	Camera::Camera(Name _name, Camera::Type _type)
		:camType(_type), name(_name)
	{
		//out("Camera(): ---------\n");
	}
	...
	void Camera::getName(Name& _name)
	{
		_name = name;
	}
	...
	void Camera::setOrientAndPosition(const Vect& inUp, const Vect& inLookAt, const Vect& inPos)
	{
		...
		this->vPos = inPos;
			...
			Trace::out("\nPosition --> (x,y,z) = (%f, %f, %f)", inPos.x(), inPos.y(), inPos.z());
			Trace::out("\nLook at  --> (x,y,z) = (%f, %f, %f)", inLookAt.x(), inLookAt.y(), inLookAt.z());
			Trace::out("\nUp       --> (x,y,z) = (%f, %f, %f)", inUp.x(), inUp.y(), inUp.z());
			Trace::out("\n");
