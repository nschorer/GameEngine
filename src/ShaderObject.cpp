...
namespace Azul
{
	...
	ShaderObject::ShaderObject(ShaderObject::Name shaderName, const char* const pShaderBaseFileName, ShaderObject::Type _type)
		: name(shaderName),
		type(_type),
		programObject()
	{
		assert(pShaderBaseFileName);
		this->privCreateShader(this->programObject, pShaderBaseFileName, type);
	}
	...
	{
		assert(this->type == ShaderObject::Type::COMPUTE);
