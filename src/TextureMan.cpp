...
namespace Azul
{
	...
	bool TextureMan::ChecksumAlreadyExists(unsigned int hash)
	{
		
		if (hash == 0) return false; // 0 means no checksum associated
	...
	void TextureMan::Add(const char *const _assetName, const Texture::Name _name, unsigned int checksum)
	{
		if (ChecksumAlreadyExists(checksum)) {
			return;
		}
		...
		// initialize it
		pNode->set(_assetName, _name, textureID, GL_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE, width, height, checksum);
